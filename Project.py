...
def Box_plot_outliers(features_scaled,target,headers):
    ...
    df.set_axis(headers[1:], axis=1,inplace=True)
...
plt.figure(figsize=(20, 15))
sns.heatmap(df.corr()[['diagnosis']].sort_values(by='diagnosis', ascending=False),linewidths=1,annot=True,cmap="coolwarm")
plt.show()
...
y.set_axis(df.columns.values[2:], axis=1, inplace=True)
